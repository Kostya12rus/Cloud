	...
	for i = 1, #all do
		...
		if SkillShot.trackTable[v.handle] and (not SkillShot.trackTable[v.handle].last or SkillShot.currentTick > SkillShot.trackTable[v.handle].last.tick) then
			if not SkillShot.trackTable[v.handle].last then
				SkillShot.trackTable[v.handle].last = {pos = v.position, tick = SkillShot.currentTick, rotR = v.rotR, time = client.gameTime, modifiers = v.modifiers}
				SkillShot.trackTable[v.handle].starightDur = 0
			else		
				local movespeed = v:GetMovespeed()
				SkillShot.trackTable[v.handle].RotSpeed = SkillShot.trackTable[v.handle].last.rotR - v.rotR
				--print(math.abs(SkillShot.trackTable[v.handle].RotSpeed))
				...
				if math.abs(SkillShot.trackTable[v.handle].RotSpeed) > 0.065 and SkillShot.trackTable[v.handle].speed then
					...
					--print(SkillShot.trackTable[v.handle].starightDur)
					if SkillShot.trackTable[v.handle].starightDur > 0.2 then
					...
					if (realspeed+100) > movespeed and #SkillShot.trackTable[v.handle].last <= #v.modifiers and realspeed/movespeed < 2 and v.activity == LuaEntityNPC.ACTIVITY_MOVE then
						local alpha = v.rotR
						SkillShot.trackTable[v.handle].speed = VectorOp.UnitVectorFromXYAngle(alpha) * (movespeed/1000)
					end
					local time = client.gameTime - SkillShot.trackTable[v.handle].last.time 
					SkillShot.trackTable[v.handle].last = {pos = v.position, tick = SkillShot.currentTick, rotR = v.rotR, time = SkillShot.trackTable[v.handle].last.time, modifiers = v.modifiers}
					SkillShot.trackTable[v.handle].starightDur = time
				end
				-- if not eff then
					-- eff = Effect(SkillShot.PredictedXYZ(v, 1000), "range_display" )
					-- eff:SetVector(1,Vector(100,0,0))
					-- eff:SetVector(0, SkillShot.PredictedXYZ(v, 1000) )
				-- else
					-- eff:SetVector(0, SkillShot.PredictedXYZ(v, 1000) )
				-- end
			end
